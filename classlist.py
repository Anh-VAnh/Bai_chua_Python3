...
def create_student_data_texfile(student_data):
    ...
    print(student_data)
    filename = "student_data.tex"
    outfile = open(filename,'w')
    count = 0
    for student in student_data:
        print(student)
        student_id, student_name, student_date_of_birth, student_training_program = student
        print(student_id, student_name, student_date_of_birth, student_training_program)
        count += 1
        outfile.write(f"""{count} & {student_id} & {student_name} & {student_date_of_birth} & {student_training_program} & & & & \\\\
\\hline
""")
...
#        print(f"Mã sinh viên '{student_id}', Họ và tên '{student_name}', Ngày sinh '{student_date_of_birth}', Chương trình đào tạo '{student_training_program}'")
        student_data.append([student_id, student_name, student_date_of_birth, student_training_program])
    datafile.close()
    return student_data
...
# Viewing classlist.pdf
    ...
    create_student_data_texfile(student_data)
